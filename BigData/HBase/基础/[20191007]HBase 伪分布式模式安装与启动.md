...
安装 HBase 之前默认我们已经完成了 Hadoop、ZooKeeper 安装，如果免费游安装可以参考如下博文：
- [http://smartsi.club/hadoop-setup-and-start.html](Hadoop 安装与启动)
- [http://smartsi.club/zookeeper-standalone-setup-and-run.html](ZooKeeper 伪集群模式安装与启动)
...
```
接下来，使用 `hdfs:////URI` 语法将 `hbase.rootdir` 从本地文件系统更改为 `HDFS` 实例的地址。在此示例中，HDFS 在本地主机上的端口8020上运行。请确保删除 `hbase.unsafe.stream.capability.enforce` 的条目或将其设置为 `true`:
```xml
<property>
  <name>hbase.rootdir</name>
  <value>hdfs://localhost:8020/hbase</value>
...
检查 HDFS 中的 HBase 目录。如果一切正常，HBase 将在 HDFS 中创建其目录。在上面的配置中，它存储在 HDFS 上的 /hbase/ 目录中。我们可以在 Hadoop 的 bin/ 目录中使用 hadoop fs 命令列出该目录。
