...
从 Spout 发出的元组可能会触发基于它产生的数千个元组。例如，考虑流式 WordCount 拓扑：
```java
...
```
这种拓扑结构从 Kestrel 队列中读取句子，将句子分解成不同的单词，然后发送每个单词之前出现过的次数。从 Spout 发出的元组触发了许多基于它的元组：句子中每个单词的元组和每个单词更新计数的元组。消息树看起来像这样：
...
当消息树遍历完，并且每一个消息都被处理完，则认为从 Spout 发出的这个 tuple 被完全处理。如果在指定的超时时间内没有处理完消息树上的消息，则认为该元组失败。可以通过 `Config.TOPOLOGY_MESSAGE_TIMEOUT_SECS` 配置指定拓扑消息处理超时时间，默认为30秒。

### 2. 消息被完全处理或未完全处理时发生什么

要理解这个问题，让我们来看看从 Spout 发出来的元组的生命周期。下面时一个 Spout 实现接口：
```java
...
```
首先，Storm 通过调用 Spout 上的 nextTuple 方法从 Spout 请求一个元组。Spout 使用 open 方法中提供的 SpoutOutputCollector 向其一个输出流中发送元组。在发送元组时，Spout 提供了一个 `消息ID`，用来在后面标识这个元组。例如，KestrelSpout 从 kestrel 队列中读取消息，并将 Kestrel 提供的 id 作为 `消息ID` 发送消息。 向 SpoutOutputCollector 发送消息如下所示：
```java
_collector.emit(new Values("field1", "field2", 3) , msgId);
```
接下来，元组被发送到消费的 Bolt 上，Storm 负责跟踪创建的消息树。如果 Storm 检测到该元组已经被完全处理，Storm 就会调用原始 Spout 任务上的 ack 方法，并带上 Spout 为 Storm 提供的消息ID。同样，如果元组超时 Storm 就会在 Spout 上调用 fail 方法。一个元组被 ack 还是 fail 都是由原来创建该元组的 Spout 任务来完成的。因此，如果 Spout 在集群上执行多个任务，是不会通过与创建它的任务不同的任务来确认 ack 或 fail。
